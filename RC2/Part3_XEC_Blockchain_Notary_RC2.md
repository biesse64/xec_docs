...
This revision preserves the documented production architecture while introducing editorial standardization aligned with Volumes 1 and 2.

## RC2 objectives

- Harmonize terminology across the documentation suite.
- Prepare unified figure, table and cross-reference numbering.
- Improve consistency of architectural descriptions.
- Prepare Release 1.0 publication.

---

<!--
Release: RC1
Review: Editorial & Technical Consolidation
...
The XEC Blockchain Notary is an independent production service developed as part of the LMWPool platform.
...
Every critical component is self-hosted:
...
This architecture guarantees complete ownership of the notarization workflow without relying on external blockchain providers.
...
Every blockchain operation is executed against the local
...
| bitcoind-xec-xec | eCash Core | Blockchain interaction |
...
The Notary follows the same architectural philosophy adopted by the mining platform.
...
This separation guarantees that failures in one environment cannot interrupt the core functionality of the other.
...
For this reason, **server.js should be considered the heart of the Notary platform.**
...
server.js executes directly on Linux.
...
Before any blockchain operation is performed,
...
Following successful notarization,
...
Among every software component composing the Blockchain Notary,

**server.js is by far the most important.**
...
SQLite is responsible for storing:
...
Each user record typically contains:
...
Each notarized document stores metadata such as:
...
Certificate records typically include:
...
an appropriate error response is generated.
...
Every endpoint returns deterministic responses.
...
This guarantees that identical documents always reference the same blockchain proof.
...
The Blockchain Integration layer connects the XEC Blockchain Notary to the **eCash blockchain** through a locally managed full node.
...
The production blockchain node is
...
is performed through authenticated JSON-RPC.
...
RPC remains completely internal.
...
# 11. System Deployment & Production Operations
...
| bitcoind-xec-xec | Docker | Container |
...
- **bitcoind-xec-xec**, providing blockchain access.
...
Although the Notary has modest hardware requirements,
...
(optional because they are reproducible)

---

## Blockchain

No blockchain backup is required.
...
Together with the mining platform documented in the previous sections, the Notary forms the second major production system of the LMWPool ecosystem.
...
Editorial consolidation completed.
...
- reserved detailed command references, configuration listings and operational runbooks for Appendices A–F. Explicit section-level cross references will be introduced before Release 1.0.
