...
This RC2 preserves the production architecture and operational procedures documented in RC1 while aligning the volume with the editorial standards adopted across the complete LMWPool documentation suite.

## RC2 objectives

- Standardize terminology across all four volumes.
- Prepare unified numbering for figures and tables.
- Prepare explicit cross-references to Appendices A–F.
...
The production platform consists of two independent business systems hosted on a single Linux server.
...
Both systems share only a limited number of infrastructure components while remaining operationally independent.

---

# 1.3 Host Host Operating System
...
Mining services, support services and blockchain notarization operate as independent subsystems sharing only the minimum required infrastructure.
...
A successful deployment concludes only when both the mining platform and the Blockchain Notary have completed their respective operational validation procedures and all public services are accessible through the production reverse proxy.

# 3. Upgrade & Change Management
...
the upgrade should be postponed.
...
Because Caddy serves both the mining platform and the Notary,

its upgrade requires additional care.
...
| Asset | Frequency |
|--------|-----------|
| Wallet | Before every major change + weekly |
| SQLite | Daily |
...
Because Caddy serves both major systems,

its availability is considered highly important.
...
|-----------|--------------|
| Linux | CPU / RAM / Disk |
| bitcoind-xec | RPC + Sync |
| CKPool | Shares + Hashrate |
...
# 7. Performance Optimization & Capacity Planning
...
Because the API primarily serves read-only data,

resource requirements remain modest.
...
SQLite performance depends primarily upon:
...
| 80 | HTTP (redirect) |
| 443 | HTTPS |
| 7333 | XEC Standard Stratum |
| 7444 | XEC Bigger Miners Stratum |
| 8333 | Bitcoin P2P |
...
This concludes **Volume 4 – Infrastructure Operations Manual** and the first complete edition of the LMWPool Technical Documentation.
...
This release harmonizes Volume 4 with the previous three volumes.

Editorial changes:
- standardized volume title and document header;
- normalized terminology and section naming;
- improved consistency across the four-volume documentation set;
- no architectural or operational changes introduced.

Detailed operational commands, configuration listings, reference material and operational runbooks are consolidated in Appendices A–F. Section-level cross references will be completed before Release 1.0.
