...
> **Document scope.** This volume documents the production services that provide statistics, public presentation, solved-block verification, notifications, market data, exchange integration and operational utilities around the XEC mining engine. The mining engine and core infrastructure are documented in Volume 1.

---

## Document Control

> **Release note:** This document is Volume 2 of the LMWPool Technical Documentation Release 1.0 set. Read it together with Volumes 1, 3 and 4 and Appendices A–F for the complete platform reference.

| Field | Value |
|---|---|
| Release | 1.0 |
| Source baseline | Volume 2 RC2 |
...
4. [Production Support Services](#4-production-support-services)
5. [Directory Layout](#5-directory-layout)
6. [Communication Model](#6-communication-model)
7. [Data Flow](#7-data-flow)
8. [Operational Philosophy](#8-operational-philosophy)
9. [`stats-api-xec`](#9-stats-api-xec)
10. [`status-page-xec`](#10-status-page-xec)
11. [`parse_block.py`](#11-parse_blockpy)
12. [`block_notify_xec.sh`](#12-block_notify_xecsh)
13. [Binance Integration and Automatic XEC Conversion](#13-binance-integration-and-automatic-xec-conversion)
14. [Market Cache and Market Data Pipeline](#14-market-cache-and-market-data-pipeline)
15. [Maintenance and Utility Scripts](#15-maintenance-and-utility-scripts)
16. [Complete Support Services Workflow](#16-complete-support-services-workflow)

---

## 1. Purpose

Unlike the mining engine, the components described in this volume do **not participate directly in proof of work**. They provide the operational capabilities that turn CKPool into a complete production platform.
...
Each component has one defined responsibility, which simplifies fault isolation and debugging.
...
Mining, statistics, notifications, presentation and administration are independent responsibilities.
...
Instead, they consume logs or use RPC.
...
- SQLite data for the Notary;
- the market cache; and
- generated JSON.
...
| Service | Purpose | Production interface |
|---|---|---|
| `stats-api-xec` | Mining statistics | Internal HTTP through Caddy routes |
| `status-page-xec` | Public website | Static content through Caddy |
| `parse_block.py` | Block verification | CKPool log and node RPC |
| `block_notify_xec.sh` | Wallet monitoring | Wallet RPC and Telegram Bot API |
| Binance utilities | XEC/BTC conversion | Binance REST API |
| Market cache | Cached market data | Static JSON through Caddy |
| Maintenance scripts | Administration | Manual or scheduled execution |

---

## 5. Directory Layout

Most support software resides in `/root/xec_miningcore`.
...
Although the directory retains the historical name `xec_miningcore`, Miningcore is **not responsible** for XEC mining; the name remains for compatibility.
...
The support services communicate through a limited and well-defined set of interfaces:

| Interface | Endpoint or data |
|---|---|
| RPC | `bitcoind-xec` |
| Filesystem | Logs and JSON files |
| HTTP | Statistics API |
| HTTPS | Caddy |
| Telegram HTTPS | Telegram Bot API |
...
Unlike CKPool, whose responsibility ends with mining, `stats-api-xec` transforms production information into JSON endpoints consumed by:
...
|---|---|
| Container | `stats-api-xec` |
| Base image | `python:3.12-alpine` |
| Status | Always running |
| Docker network | `poolnet` |
| Internal port | `8080` |
...
The Statistics API acts as the single source of operational information for the public interface.
...
|---|---|
| Pool | `/api/pools/xec` |
| Miners | `/api/pools/xec/miners` |
| Statistics | `/stats/*` |
| Worker summary | `/stats/workers-summary` |
| Worker details | `/stats/miners-workers` |
| Live workers | `/stats/miner-workers-live` |
...
Notice that the Statistics API never parses CKPool logs directly.
...
- centralized TLS;
- centralized routing;
- a simplified firewall configuration; and
...
The Statistics API should never expose:
...
Unlike the Statistics API, the Status Page contains **no business logic**.
...
Everything dynamic originates from the Statistics API.
...
These values are retrieved from the Statistics API and rendered in real time.
...
The `status-page-xec` service therefore acts exclusively as the visual interface of the LMWPool XEC platform, leaving operational processing to the underlying support services.
## 11. `parse_block.py`

### 11.1 Purpose

The `parse_block.py` utility independently verifies every block reported by CKPool before it becomes visible on the public website.
...
This ensures that only blocks confirmed through the local node are published.
...
This architecture prevents candidates that fail blockchain verification from being published.
...
A block is considered valid only if all of the following conditions are satisfied.
...
- protection against browsers reading incomplete files;
- safe updates; and
...
This behavior avoids accidental publication of incomplete information.
...
- CKPool, through its log file; and
- `bitcoind-xec`, through RPC.
...
Although relatively small, `parse_block.py` is one of the key reliability components of the LMWPool XEC platform.
...
The `block_notify_xec.sh` service detects newly generated mining rewards and notifies administrators through Telegram.
...
This design bases notifications on coinbase transactions recorded by the blockchain node rather than on mining-log events.
...
This mechanism provides duplicate protection so that previously processed rewards are not notified again.
...
Although implemented as a shell script, `block_notify_xec.sh` is one of the platform's critical operational monitoring components.
...
## 13. Binance Integration and Automatic XEC Conversion
...
For this reason, the exchange layer is an **auxiliary service** that remains separate from the mining engine.
...
- `bitcoind-xec` continues synchronizing.
...
The integration uses no browser automation, manual interaction or screen scraping.
...
Failures inside the exchange layer do not propagate to:
...
The Binance integration currently consists of the following utility:
...
It uses a Python runtime, HTTPS communication and the Binance REST API.
...
Recommended permissions, subject to the deployed operational workflow:
...
## 14. Market Cache and Market Data Pipeline
...
The market cache therefore acts as an intermediate layer between external providers and the public Status Page.
...
The mounted cache remains available across Caddy restarts.
...
The cache should not be overwritten directly.
...
The Market Cache is independent from:
...
Consequently, failures within the market subsystem do not affect mining operations.
...
## 15. Maintenance and Utility Scripts
...
Every production utility should generate its own log. Recommended characteristics include:
...
- minimal external dependencies; and
...
Because every utility remains independent, future development does not require changes to CKPool or `bitcoind-xec`.
...
| `block_notify_xec.sh` | Wallet monitoring and Telegram notifications |
...
```text
       ...
       ▼                 ▼                +---------------+
+---------------+  +---------------+       | bitcoind-xec  |
|status-page-xec|  | XEC Notary    |──────▶| eCash Core    |
+-------+-------+  +-------+-------+  RPC  +-------+-------+
        │                  │ SQLite               │
        ▼                                          ▼
+---------------+                           eCash blockchain
...
+--------+---------+
         ...
         │
+--------+---------+        +-------------------------+
| parse_block.py   |◀───────| ckpool.log from CKPool |
+------------------+        +-------------------------+
```
...
| Component | Primary responsibility | Criticality |
|---|---|---|
| `bitcoind-xec` | Blockchain consensus | Critical |
| CKPool | Mining engine | Critical |
| Caddy (`caddy-btc`) | HTTPS reverse proxy | Critical to public services |
| `stats-api-xec` | Statistics | Non-critical to mining |
| `status-page-xec` | Web UI | Non-critical to mining |
| `parse_block.py` | Block verification | Non-critical to mining |
| `block_notify_xec.sh` | Telegram alerts | Non-critical to mining |
| Market Cache | Market prices | Non-critical to mining |
| Binance Utilities | Exchange automation | Non-critical to mining |
| Blockchain Notary | Notarization | Independent from mining |
...
|---|---|
| CKPool | `bitcoind-xec` |
| `parse_block.py` | CKPool and `bitcoind-xec` |
| `stats-api-xec` | JSON files |
| `status-page-xec` | `stats-api-xec` |
| Caddy (`caddy-btc`) | Backend containers |
...
1. Start `bitcoind-xec`.
2. Wait for blockchain synchronization.
3. Start CKPool.
4. Run `parse_block.py`.
5. Start `stats-api-xec`.
6. Start `status-page-xec`.
7. Start Caddy.
8. Start `block_notify_xec.sh`.
9. Start the Blockchain Notary.
10. Start the Binance utilities.
...
### 16.16 Volume 2 Summary

The support services documented in this volume transform the core mining engine into a complete production platform.
...
Volume 3, *XEC Blockchain Notary*, documents the Notary's Node.js architecture, SQLite database, REST API, blockchain integration, certificate generation, email workflow and operational procedures.

---

## Release 1.0 Editorial Notes

Release 1.0 consolidates the Volume 2 RC2 source without changing the production architecture or operational configuration.
...
Detailed inventories, configuration listings, operational commands and runbooks remain in Appendices A–F.
