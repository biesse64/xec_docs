...
> **Document scope.** This appendix inventories the deployed production infrastructure that supports the XEC Mining Platform and XEC Blockchain Notary. It describes the real production environment rather than a conceptual reference architecture.

---

## Document Control

> **Release note:** This document is Appendix A of the LMWPool Technical Documentation Release 1.0 set. Read it together with Volumes 1–4 and Appendices B–F for the complete platform reference.

| Field | Value |
|---|---|
| Release | 1.0 |
| Source baseline | Appendix A RC2 |
| Intended audience | Platform owner, system administrators, technical maintainers |
| Change policy | Production infrastructure changes must be reflected in this inventory |
| Architectural authority | Running production configuration and verified operational evidence |

## Document Purpose

This appendix provides the infrastructure inventory for the deployed LMWPool production environment.

Volumes 1–4 describe the platform architecture, software components and operational procedures. This appendix inventories the corresponding production assets.
...
The information in this appendix is derived from the production environment inventory rather than theoretical examples. The operational context for these assets is documented in Volumes 1–4.

---

## Table of Contents

1. [Executive Summary](#a1-executive-summary)
2. [Host Infrastructure](#a2-host-infrastructure)
3. [Operating System](#a3-operating-system)
4. [Hardware Inventory](#a4-hardware-inventory)
5. [Filesystem Layout](#a5-filesystem-layout)
6. [Docker Architecture](#a6-docker-architecture)
7. [Container Inventory](#a7-container-inventory)
8. [Docker Networks](#a8-docker-networks)
9. [Published Services](#a9-published-services)
10. [Port Inventory](#a10-port-inventory)
11. [Storage Inventory](#a11-storage-inventory)
12. [Configuration Inventory](#a12-configuration-inventory)
13. [Native Services](#a13-native-services)
14. [Security Inventory](#a14-security-inventory)
15. [Backup Scope](#a15-backup-scope)
16. [Disaster Recovery Priority](#a16-disaster-recovery-priority)
17. [Infrastructure Statistics](#a17-infrastructure-statistics)
18. [Appendix Summary](#a18-appendix-summary)
...
The inventory presented in this appendix represents the production environment at the time of generation and should be considered the authoritative infrastructure reference.

---

### A.1.2 Infrastructure Overview

The production platform comprises four major infrastructure layers.
...
These principles are reflected throughout Volumes 1–4.
...
This server hosts both the mining infrastructure and the Blockchain Notary. Both systems share the same production VPS while remaining logically independent.
...
The operating system is maintained as a Long-Term Support (LTS) release to maximize platform stability.
...
Kernel updates should always be validated against Docker and blockchain services before deployment into production.
...
The available processing capacity is sufficient for concurrent execution of the mining platform, support services and blockchain notarization environment.
...
This organization simplifies maintenance, backup procedures and disaster recovery activities.
...
Each directory has a single operational responsibility and should remain independent whenever possible.
...
The production platform uses distinct public traffic paths. HTTPS traffic is reverse proxied by Caddy, while Stratum traffic connects directly to CKPool.

```text
Internet
├── HTTPS ───────► Caddy Reverse Proxy ─────► HTTPS application services
│                                                   │
└── Stratum ────────────────────────────────► CKPool │
                                                   │
                         Docker networks / local service paths
                                                   │
                                                   ▼
                                              `bitcoind-xec`
```

Only explicitly published services are accessible from outside the host. Stratum mining traffic never traverses Caddy; only HTTPS traffic is reverse proxied through Caddy.

All internal communications remain confined to Docker networks.
...
Automatic restart policies reduce operational downtime while preserving deterministic recovery behavior.
...
Additional containers supporting other platform components may coexist on the host but remain logically independent from the XEC mining platform.
...
The production inventory identifies multiple Docker networks supporting the platform.
...
The public communication paths are:

```text
Internet
├── HTTPS ───────► Caddy Reverse Proxy ─────► Stats API / Notary API
│                                                   │
└── Stratum ────────────────────────────────► CKPool │
                                                   │
                                                   ▼
...
The browser never communicates directly with blockchain services, and Stratum traffic connects directly to CKPool rather than through Caddy.
...
All remaining services communicate exclusively over internal Docker networks or local interfaces.
...
This approach simplifies firewall administration while reducing unnecessary exposure.
...
| `7333` | TCP | CKPool Standard Variable Difficulty endpoint |
| `7444` | TCP | CKPool High Difficulty endpoint |
...
The inventory collected from the production environment identifies these files as the primary operational configuration assets.
...
Native services intentionally remain lightweight and independent from application business logic.
...
The inventory confirms that production services are managed through systemd where appropriate.
...
Backend infrastructure remains isolated through Docker networking and host firewall configuration.
...
The resulting architecture is intentionally simple while supporting multiple production workloads.
...
The Infrastructure Inventory is the definitive asset reference for the production environment supporting the LMWPool platform.
...
Future revisions should use updated production inventory data to maintain alignment between this appendix and the deployed environment.
